...
class RNNRWW(AbstractNMM):
    ...
    sc: tensor node_size x node_size array
    ...
    def __init__(self, node_size: int,
        ...
        # self.sc = sc.clone() if isinstance(sc, torch.Tensor) else torch.tensor(sc, dtype=torch.float32)
        self.register_buffer('sc', sc.clone())
        self.register_buffer('sc_fitted', sc.clone())
    ...
    def createIC(self, ver):
        ...
        return torch.tensor(0.2 * np.random.uniform(0, 1, (self.node_size, self.state_size)) + np.array(
                [0, 0, 0, 1.0, 1.0, 1.0]), dtype=torch.float32, device=DEVICE)
...
def setModelParameters(model):
    ...
    if model.use_Gaussian_EI:
        model.E_m = Parameter(torch.tensor(0.16, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.E_m)
        model.I_m = Parameter(torch.tensor(0.1, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.I_m)
        # model.f_m = Parameter(torch.tensor(1.0, dtype=torch.float32, device=DEVICE))
        model.v_m = Parameter(torch.tensor(1.0, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.v_m)
        # model.x_m = Parameter(torch.tensor(0.16, dtype=torch.float32, device=DEVICE))
        model.q_m = Parameter(torch.tensor(1.0, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.q_m)

        model.E_v_inv = Parameter(torch.tensor(2500, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.E_v_inv)
        model.I_v_inv = Parameter(torch.tensor(2500, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.I_v_inv)
        # model.f_v = Parameter(torch.tensor(100, dtype=torch.float32, device=DEVICE))
        model.v_v_inv = Parameter(torch.tensor(100, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.v_v_inv)
        # model.x_v = Parameter(torch.tensor(100, dtype=torch.float32, device=DEVICE))
        model.q_v_inv = Parameter(torch.tensor(100, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.v_v_inv)
    ...
    if model.use_Bifurcation:
        model.sup_ca = Parameter(torch.tensor(0.5, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.sup_ca)
        model.sup_cb = Parameter(torch.tensor(20, dtype=torch.float32, device=DEVICE))
        param_hyper.append(model.sup_cb)
        model.sup_cc = Parameter(torch.tensor(10, dtype=torch.float32, device=DEVICE))
    ...
    if model.use_fit_gains:
        model.gains_con = Parameter(torch.tensor(np.zeros((model.node_size, model.node_size)) + 0.05,
                                                 dtype=torch.float32, device=DEVICE))  # connenction gain to modify empirical sc
        param_reg.append(model.gains_con)
    else:
        # model.gains_con = torch.tensor(np.zeros((model.node_size, model.node_size)), dtype=torch.float32, device=DEVICE)
        model.register_buffer("gains_con", torch.zeros((model.node_size, model.node_size), dtype=torch.float32, device=DEVICE))
...
def integration_forward(model, external, hx, hE):
    ...
    dt = torch.tensor(model.step_size, dtype=torch.float32, device=DEVICE)

    # Update the Laplacian based on the updated connection gains gains_con.
    if model.sc.shape[0] > 1:
        gains = model.gains_con.to(DEVICE)
        sc_base = model.sc.to(DEVICE)
        sc_mod = torch.exp(gains) * sc_base
        # Update thef Laplacian based on the updated connection gains gains_con.
        # sc_mod = torch.exp(model.gains_con) * torch.tensor(model.sc, dtype=torch.float32)
        # sc_mod = torch.exp(model.gains_con) * model.sc.clone().detach().float() # fixed
        ...
        lap_adj = torch.tensor(np.zeros((1, 1)), dtype=torch.float32, device=DEVICE)
    ...
    # placeholders for output BOLD, history of E I x f v and q
    bold_window = torch.zeros((model.node_size, model.TRs_per_window), device=DEVICE)
    # E_window = torch.zeros((model.node_size,model.TRs_per_window))
    # I_window = torch.zeros((model.node_size,model.TRs_per_window))

    x_window = torch.zeros((model.node_size, model.TRs_per_window), device=DEVICE)
    f_window = torch.zeros((model.node_size, model.TRs_per_window), device=DEVICE)
    v_window = torch.zeros((model.node_size, model.TRs_per_window), device=DEVICE)
    q_window = torch.zeros((model.node_size, model.TRs_per_window), device=DEVICE)

    E_hist = torch.zeros((model.node_size, model.TRs_per_window, model.steps_per_TR), device=DEVICE)
    I_hist = torch.zeros((model.node_size, model.TRs_per_window, model.steps_per_TR), device=DEVICE)
    E_mean = hx[:, 0:1]
    I_mean = hx[:, 1:2]
    ...
    if model.use_dynamic_boundary:
        for TR_i in range(model.TRs_per_window):
            ...
            for step_i in range(model.steps_per_TR):
                E = torch.zeros((model.node_size, model.sampling_size), device=DEVICE)
                I = torch.zeros((model.node_size, model.sampling_size), device=DEVICE)
                for sample_i in range(model.sampling_size):
                    E[:, sample_i] = E_mean[:, 0] + 0.02 * torch.randn(model.node_size, device=DEVICE)
                ...
                E_next = E + dt * (-E * torch.reciprocal(tau_E) + gamma_E * (1. - E) * rE) \
                         + torch.sqrt(dt) * torch.randn(model.node_size, model.sampling_size, device=DEVICE) * std_in  ### equlibrim point at E=(tau_E*gamma_E*rE)/(1+tau_E*gamma_E*rE)
                I_next = I + dt * (-I * torch.reciprocal(tau_I) + gamma_I * rI) \
                         + torch.sqrt(dt) * torch.randn(model.node_size, model.sampling_size, device=DEVICE) * std_in
        ...
        for TR_i in range(model.TRs_per_window):
            ...
            for step_i in range(model.steps_per_TR):
                ...
                q_next = q + 1 * dt * (f * (1 - torch.pow(1 - rho, torch.reciprocal(f))) * torch.reciprocal(rho) \
                         - q * torch.pow(v, torch.reciprocal(alpha) - 1)) * torch.reciprocal(tau_0)
                # q_next = q + 1 * dt * (f * (1 - torch.pow(1 - rho, torch.reciprocal(f))) * torch.reciprocal(rho) \
            ...
            # print(f"[{(TR_i):f}]  E={E.mean():6.3f} I={I.mean():6.3f} I_E={IE.mean():6.3f} I_I={II.mean():6.3f} R_E={rE.mean():6.3f} R_I={rI.mean():6.3f}", end="  |  ")
            # print(f"x={x.mean():6.3f} f={f.mean():6.3f} v={v.mean():6.3f} q={q.mean():6.3f}") # [HAEMO {step_idx}/{params.hemo_steps_per_tr}] 

            # Put the BOLD signal each tr to the placeholder being used in the cost calculation.
            bold_window[:, TR_i] = (std_out * torch.randn(model.node_size, 1, device=DEVICE) +
        ...
        for TR_i in range(model.TRs_per_window):
            ...
            for step_i in range(model.steps_per_TR):
                ...
                I = torch.zeros((model.node_size, model.sampling_size), device=DEVICE)
                for sample_i in range(model.sampling_size):
                    E[:, sample_i] = E_mean[:, 0] + 0.001 * torch.randn(model.node_size, device=DEVICE)
                    I[:, sample_i] = I_mean[:, 0] + 0.001 * torch.randn(model.node_size, device=DEVICE)
