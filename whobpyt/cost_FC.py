...
class CostsFC(AbstractLoss):
    ...
    def loss(self, simData: dict, empData: torch.Tensor):
        ...
        logits_series_tf = sim.to(DEVICE)
        labels_series_tf = empData.to(DEVICE)
        ...
        FC_T = FC_T.to(DEVICE)
        FC_sim_T = FC_sim_T.to(DEVICE)


        # Masking out the upper triangle of the FC matrix and keeping the lower triangle
        ones_tri = torch.tril(torch.ones_like(FC_T), -1).to(DEVICE)
        zeros = torch.zeros_like(FC_T, device=DEVICE)  # create a tensor all ones
...
class CostsFixedFC(AbstractLoss):
    ...
    def loss(self, simData, empData):
        ...
        losses_corr: torch.tensor
            cost function value
        """
        simTS = simData[self.simKey].to(self.device)
        empFC = empData.to(self.device)
