...
class par:
    ...
    def __init__(self, val, prior_mean = None, prior_var = None, fit_par = False, fit_hyper = False, asLog = False, isPlastic = False, device = DEVICE):
        '''
    ...
    def randSet(self):
        ...
        if self.has_prior:
            self.var = self.prior_mean.detach() + self.prior_var.detach() * torch.randn(1).to(self.device)
            if self.fit_par:
                self.val = torch.nn.parameter.Parameter(self.val).to(self.device)
        else:
